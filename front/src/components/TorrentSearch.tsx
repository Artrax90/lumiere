...
export default function TorrentSearch({ title, onPlay }: TorrentSearchProps) {
    ...
    try {
      ...
      if (data.error) {
        ...
        if (data.files.length === 1) {
          ...
          onPlay(data.files[0].hlsUrl || data.files[0].streamUrl, episodeName, data.files[0].externalSubs || [], data.files[0].directUrl, data.files[0].hlsUrl);
        } else {
          setFiles(data.files);
        }
      } else {
  ...
  const playFile = (file: TorrentFile) => {
    ...
    onPlay(file.hlsUrl || file.streamUrl, episodeName, (file as any).externalSubs || [], file.directUrl, file.hlsUrl);
  };
