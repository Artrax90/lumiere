...
export default function MyView({ onSelect }: MyViewProps) {
  ...
  return (
    <div className="min-h-screen w-full max-w-full overflow-x-hidden px-4 pt-20 pb-24 sm:px-6 md:px-8 lg:px-14">
      ...
      {/* Tabs: Responsive Segmented Control without scale overflow */}
        ...
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 2xl:grid-cols-7">
          {items.map((title) => (
            <div key={title.id} className="group relative">
              <Card title={title} onSelect={onSelect} />
              <button
                onClick={(e) => handleRemove(e, title.id)}
                title="Удалить из списка"
                className="absolute top-2 right-2 z-20 flex h-8 w-8 items-center justify-center rounded-full bg-black/60 text-white/70 backdrop-blur-md opacity-0 transition-opacity hover:bg-red-500 hover:text-white group-hover:opacity-100"
              >
